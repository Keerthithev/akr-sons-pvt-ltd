...
async function cleanupOldSeparateEntries() {
  try {
    // Connect to MongoDB
    ...
    for (const vac of vacs) {
      ...
      // Find and delete the old separate payment entries based on details pattern
      const oldEntries = await AccountData.find({
        details: { 
          $regex: `(Advance Payment|Down Payment|Registration Fee|Document Charge) Collection - Vehicle Allocation Coupon ${vac.couponId}`, 
          $options: 'i' 
        }
      });
      ...
      oldEntries.forEach(entry => {
        const match = entry.details.match(/(Advance Payment|Down Payment|Registration Fee|Document Charge)/);
        const entryType = match ? match[1] : 'Unknown';
        console.log(`    - ${entryType}: LKR ${Math.abs(entry.amount).toLocaleString()}`);
      });
    ...
    const totalOldEntries = await AccountData.countDocuments({ 
      details: { $regex: '(Advance Payment|Down Payment|Registration Fee|Document Charge) Collection - Vehicle Allocation Coupon', $options: 'i' }
    });
    ...
    const remainingEntries = await AccountData.find({}).sort({ date: -1 }).limit(15);
